...
```{r}
#x axis
ei = c(0.4,0.5,0.6,0.7,0.8)  
#y axis
ec = c(0.04,0.05,0.06,0.07,0.08)
#z axis
#biomass
St=2000
Total_ec=19.4

z=matrix(NA,nrow=5,ncol=5)
for (i in 1:5){
  for (j in 1:5){
    Ei = ei[i]
    Ec= ec [j]
    z[i,j]=St*0.478*Ei*Ec/Total_ec
  }
}

#This function draws perspective plots of a surface over the x--y plane.
persp(ei, ec, z, phi = 45, theta = 45,
      xlab = "Ei", ylab = "Ec",
      main = "Biomass (carbon) of the whole plant")
```
...
scatter3D(data$x, data$y, data$z, phi = 0, bty = "g",
          ...
          main="Yield potential, Ei and Ec")
